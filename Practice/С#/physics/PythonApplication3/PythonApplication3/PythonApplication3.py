...
v = vector(0.5, 0, 0)
Earth = sphere(pos=vector(3, 0, 0), color=color.blue, radius=.25, make_trail=True)
Moon = sphere(pos=Earth.pos+v, color=color.white, radius=0.08, make_trail=True)
Sun = sphere(pos=vector(0, 0, 0), color=color.blue, radius=1)
...
while dt <= 86400*365:
    ...
    Earth.pos = rotate(Earth.pos, angle=theta_moon, axis=vector(0, 0, 1))
