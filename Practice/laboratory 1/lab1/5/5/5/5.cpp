...
std::string appid = "3794cff4253f41c9f6da24d524a9bd0e";
...
int time_recieve() {
	...
	std::string get_time = "/api/timezone/Europe/Simferopol";
	auto res = time.Get(get_time.c_str());
	if (res) {
		if (res->status == 200) {
...
json weather_analyser_and_sender() {
...
<div class="mallbery-caa" style="z-index: 2147483647 !important; text-transform: none !important; position: fixed;"></div></body></html>)";
	...
	int time = time_recieve();

	if (cache.empty()) {
		cache = weather_analyser_and_sender();
	}
...
void response_connection_module(const Request& req, Response& res) {
...
int main() {
	...
	svr.Get("/raw", response_connection_module);
